...
// MOSI				PA7 				SPI1_MOSI	主设备输入从设备输出引脚
// MISO				PA6					SPI1_MISO	主设备输出从设备输入引脚
// IRQ				-						中断引脚（未使用）
// GND				GND					接地
...
void MFRC522_WriteRegister(uint8_t addr, uint8_t val) {
    addr = (addr << 1) & 0x7E; // 地址格式: 0XXXXXX0
...
uint8_t MFRC522_Check(uint8_t* id) {
    uint8_t status;
    status = MFRC522_Request(PICC_REQIDL, id); // 查找卡片，返回卡片类型
    if (status == MI_OK) status = MFRC522_Anticoll(id); // 卡片检测到。防冲突，返回4字节的卡片序列号
...
uint8_t MFRC522_ToCard(uint8_t command, uint8_t* sendData, uint8_t sendLen, uint8_t* backData, uint16_t* backLen) {
    ...
    if (command == PCD_TRANSCEIVE) MFRC522_SetBitMask(MFRC522_REG_BIT_FRAMING, 0x80); // StartSend=1，开始发送数据 
    ...
    if (i != 0)  {
        if (!(MFRC522_ReadRegister(MFRC522_REG_ERROR) & 0x1B)) {
            ...
            if (command == PCD_TRANSCEIVE) {
                ...
                for (i = 0; i < n; i++) backData[i] = MFRC522_ReadRegister(MFRC522_REG_FIFO_DATA); // 读取FIFO中的接收到的数据
            }
        } else status = MI_ERR;
    }
    return status;
...
void MFRC522_CalculateCRC(uint8_t* pIndata, uint8_t len, uint8_t* pOutData) {
    ...
    MFRC522_SetBitMask(MFRC522_REG_FIFO_LEVEL, 0x80); // 清除FIFO指针
...
void MFRC522_Init(void) {
    ...
    MFRC522_WriteRegister(MFRC522_REG_RF_CFG, 0x70); // 48dB增益	
